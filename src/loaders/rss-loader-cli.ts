...
import { RSSLoader, RSSFeedConfig } from './rss.loader.js';
...
program
  .name('rss-loader')
  .description('LangChain RSS Document Loader CLI')
  .version('1.0.0');
  ...
  .action(async (options) => {
    try {
      ...
      result.rows.forEach((row, index) => {
        ...
        const lastFetch = lastFetched ? ` (last: ${lastFetched.toLocaleDateString()})` : '';
        
        console.log(`  ${status} ${name}${lastFetch}`);
        console.log(`     ${url}`);
      });
...
program
  .command('test')
  .description('Test RSS feed accessibility')
  .requiredOption('-u, --url <url>', 'RSS feed URL to test')
  .option('-n, --name <name>', 'Feed name for display', 'Test Feed')
  .action(async (options) => {
    try {
      console.log(`🧪 Testing RSS feed: ${options.name}`);
      ...
      if (documents.length > 0) {
        ...
        console.log(`Title: ${sample.metadata.title}`);
        console.log(`URL: ${sample.metadata.url}`);
        console.log(`Published: ${sample.metadata.publishedAt || 'Unknown'}`);
        console.log(`Content length: ${sample.pageContent.length} characters`);
      }
