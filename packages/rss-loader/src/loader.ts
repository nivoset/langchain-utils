 ...
 * RSS Document Loader
 * 
 * Loads RSS feeds and converts them to Document objects
...
export class RSSLoader {
  ...
  /**
   * Load RSS feed and convert to Documents
  ...
  private async checkArticleExists(article: RSSArticle): Promise<boolean> {
    try {
      const result = await client.execute({
        sql: 'SELECT id FROM articles WHERE url = ? OR (title = ? AND source = ?)',
        args: [article.url, article.title, article.source]
      });
  ...
  static async loadMultiple(
    ...
    for (const config of configs) {
      try {
        ...
        console.error(`Error loading RSS feed ${config.name}:`, error);
      }
    }
  ...
  static async loadFromDatabase(
    ...
    try {
      let sql = 'SELECT name, url, category FROM rss_feeds WHERE is_active = 1';
      const args: any[] = [];
